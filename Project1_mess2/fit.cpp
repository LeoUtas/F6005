...
//make some containers;  
  ...
  for(i = 0;i < ns;++i){ 
    ...
    for(j = 0;j < L;++j){  
      for(k = 0;k < A;++k){plas(i,j,k)=temp(j,k);}
    }
  }
...
//compute F; matrix exp() does not work 
  for(j = 0;j < Y;++j){ 
    F(0,j) = zero;  
    for(i = 1;i < A;++i){
        F(i,j) = exp(log_F_main + log_F_dev(i-1,j));
  }}
  Z = F + M;
  
  vector<Type> log_Rec = log_Rec_dev + log_meanR;
  
  //A prior on the initial age distribution; steady state with Z=0.2;
  for(i = 1;i < A-1;++i){nll -= dnorm(log_N0(i)-log_N0(i-1),Type(0.2),Type(0.5),true);}  
  //A prior on the initial age distribution; steady state with Z=0.2;
  //nll -= dnorm(log_F_main,log(Type(0.1)),Type(2.0),true);
...
//compute beginning of year pop num@len (NL), catch@len (CL), and survey num@len (NLs);
  ...
  for(k = 0;k < ns;++k){
    ...
    for(i = 0;i < L;++i){for(j = 0;j < A;++j){temp1(i,j) = plas(k,i,j);}}   
    for(i = 0;i < A;++i){for(j = 0;j < Y;++j){temp2(i,j) = Ns(k,i,j);}}
...
//compute biomass, catch biomass and ssb @ len, and total biomass and ssb;
  ...
  //logq 
  for(k = 0;k < ns;++k){ 
  for(i = 3;i < 15;++i){nll -= dnorm(logq(k,i),logq(k,i-1),Type(0.3),true);}
  for(i = 15;i < 30;++i){nll -= dnorm(logq(k,i),logq(k,i-1),std_logq(k),true);}
  for(i = 30;i < 45;++i){nll -= dnorm(logq(k,i),logq(k,i-1),Type(0.3),true);}} 
  ...
  array<Type> log_F_dev1(A-2,Y); // A-2 because F_1 = 0 , and F_A = F_A-1;
  for(i = 0;i < A-2;++i){ 
