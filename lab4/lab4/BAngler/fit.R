setwd("D:/OneDrive - University of Tasmania/HOANG.Ng84/Education/Hoang.MUN20/Required courses/Fish 6005/labs/F6005/lab4")
...
dat = list(year=tmb.data$year,C = tmb.data$C,log_C = tmb.data$log_C,
...
index.name = aggregate(tmb.data$name,list(iq=dat$iq),unique)$x
...
start.q = aggregate(tmb.data$index,list(iq=tmb.data$iq),mean)
...
//system.time(
//opt<-nlminb(obj$par,obj$fn,,lower=lower,upper=upper,
//control = list(trace=0,iter.max=5000,eval.max=10000)) )
