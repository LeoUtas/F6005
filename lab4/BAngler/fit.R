...
dat <- list(
  year = tmb_data$year, C = tmb_data$C, log_C = tmb_data$log_C,
  index = tmb_data$index, log_index = tmb_data$log_index,
  iyear = tmb_data$iyear + 1, iq = tmb_data$iq + 1
)
dat$istd <- dat$iq
...
index.name <- aggregate(tmb_data$name, list(iq = dat$iq), unique)$x
parm.name <- c(paste("q(1000s)", index.name), "r", "K", paste("sigma", index.name))
out <- parms[, 3:4]
...
for (i in 1:length(index.name)) {
  ...
  ind <- tmb_data$name == u
  iyear <- tmb_data$iyear[ind] + 1
  index <- tmb_data$index[ind] / scale
  ...
  xlim <- range(tmb_data$year)

  xi <- tmb_data$year[iyear]
  yi <- rep(NA, length(xi))
  pyi <- yi
  pres <- yi
  ind1 <- xi %in% tmb_data$year
  yi[ind1] <- index
  pyi[ind1] <- Eindex
  pres[ind1] <- std_resid
...
exploit <- tmb_data$C / biomass
rexploit <- exploit / Hmsy

ylim <- range(0, rbiomass, rexploit)

par(mar = c(4, 2, 2, 1), mgp = c(2.5, 1, 0))
plot(tmb_data$year, rbiomass, xlab = "Year", ylab = "", type = "n", ylim = ylim)
lines(tmb_data$year, rbiomass, col = "red", type = "b", lwd = 2)
lines(tmb_data$year, rexploit, col = "blue", type = "b", lwd = 2)
...
start.q <- aggregate(tmb_data$index, list(iq = tmb_data$iq), mean)

parameters <- list(
  log_r = log(0.2),
  log_K = log(30),
  log_q = log(start.q$x) - log(15),
  log_Po = log(0.6),
  log_Ho = log(0.1),
  log_sd_rw = log(0.2),
