...
## cod to map meanF's by age and for two time periods
...
t.L = parameters.L
t.L$log_len_o=NULL  
t.L$log_F_main=rep(-Inf,length(levels(map$log_F_main))) 
...
t.U = parameters.U
t.U$log_len_o=NULL  
t.U$log_F_main=rep(Inf,length(levels(map$log_F_main))) 
...
ind = abs(CLc.vec$resid) > 10
CLc.vec[ind,]

ind1 = CLc.vec$Year==1993    
CLc.vec[ind1,]
