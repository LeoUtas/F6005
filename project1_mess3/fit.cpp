...
//compute F; matrix exp() does not work 
  ...
  for(i = 1;i < A-2;++i){nll -= dnorm(log_N0(i)-log_N0(i-1),-Type(0.2),Type(0.5),true);} 
  ...
  for(j = 1;j < Y;++j){
    ...
    for(i = 1;i < A;++i){
      ...
      if(i==A-1){log_N(i,j) = log(exp(log_N(i-1,j-1)-Z(i-1,j-1))+exp(log_N(i,j-1)-Z(i,j-1)))+pe(i-1,j-1);} //plus group
    } 
  }   
...
//compute biomass, catch biomass and ssb @ len, and total biomass and ssb;
  ...
  //logq 
  for(k = 0;k < ns;++k){ 
  for(i = 3;i < 15;++i){nll -= dnorm(logq(k,i)-logq(k,i-1),zero,Type(0.2),true);}
  for(i = 15;i < 30;++i){nll -= dnorm(logq(k,i),logq(k,i-1),std_logq(k),true);}
  for(i = 30;i < 45;++i){nll -= dnorm(logq(k,i),logq(k,i-1),Type(0.2),true);}  
}
