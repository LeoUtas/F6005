...
parameters <- list(
  ...
  logit_ar = rep(3, 4),
  dev = matrix(0, nrow = Y, ncol = A, byrow = TRUE)
)
...
P2 <- xyplot(pred ~ year | as.factor(age),
  groups = group,
...
SW.pred.old <- matrix(mean.old, nrow = length(year.old), ncol = ncol(SW.pred), byrow = TRUE)
...
yr1 <- cbind(tapply(Wo.vec$index, Wo.vec$age, min, na.rm = TRUE), tapply(Wo.vec$index, Wo.vec$age, max, na.rm = TRUE))
